...
embeddings = OpenAIEmbeddings(
    ...
    chunk_size=50,
    # when small faster responselower memory/cpu and more req will needed
    retry_min_seconds=10,
    #
)
#chroma = Chroma(persist_directory="chroma_db", embedding_function=embeddings)
vectorstore = PineconeVectorStore(
    index_name="langchain-doc-index2", embedding=embeddings
)
tavily_crawl = TavilyCrawl()
#this will get the url and take the documentation from it
...
async def main():
    ...
    tavily_crawl_results = tavily_crawl.invoke(
        input={
            ...
            # advanced is hgiher success rate maybe increase the latency
            "instructions": "Documentation relevant to ai agents",
            #when crawler scrape pages it helps to get specific pages
            "max_depth": 5,
            #longer max depth longer run time, start small then increase based on needs. Smaller is cheaper
            # bigger number makes you crawl more pages, it will be slower
        }
